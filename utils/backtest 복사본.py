...
def check_adjusted_difference(df: pd.DataFrame) -> bool:
    """
    Tiingo 데이터프레임에서 조정 종가(adjClose)와 원본 종가(close)가 다른지 확인
    - 다르면: True (조정 데이터 있음)
    - 같으면: False (조정 없음)
    """
    if "adjClose" not in df.columns or "close" not in df.columns:
        print("❌ 필요한 열이 없습니다.")
        return False
    
    # 결측치 제거 후 비교
    comp_df = df[["adjClose", "close"]].dropna()
    has_diff = not (comp_df["adjClose"] == comp_df["close"]).all()

    if has_diff:
        print("✅ 조정된 가격이 적용된 데이터입니다.")
    else:
        print("⚠️ 조정된 가격과 원본 가격이 동일합니다. 배당/분할 정보가 없을 수 있습니다.")
